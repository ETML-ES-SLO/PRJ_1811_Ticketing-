    ...
    switch (MenuList)
    {
        ...
        case Idle:
        {
            ...
            if(appButtons.But_ACC)
            {
                ...
                {
                    if(Students_Info[I].ID == List[0])
                    {
                        SendMessage(Students_Info[I].ID,TICKET_ACCEPT);
                        Students_Info[I].LimitQues --;
                        for(J = 0; J < (Nb_Students_In_List); J++)
                        {
                            List[J] = List[J+1];
                        }                   
                        List[Nb_Students_In_List-1] = 0;
                        Nb_Students_In_List--;    
                    }
                }
                TFT_Font_List();
            ...
            if(appButtons.But_DECL)
            {
                for(I = 0; I < I_Nbstudents; I ++)
                {
                    if(Students_Info[I].ID == List[0])
                    {
                        SendMessage(Students_Info[I].ID,TICKET_REFUSE);
                        Students_Info[I].LimitQues --;
                        for(J = 0; J < (Nb_Students_In_List); J++)
                        {
                            List[J] = List[J+1];
    ...
    switch (init_status)
    {
        ...
        case 4:
        {
            for(I = 0; I < I_Nbstudents; I ++)
            {
                ...
                AddTarget.val32 = Students_Info[I].ID;
                ...
                AddTarget.val32 =  Students_Info[I].ID;
                SendMessage(AddTarget.val32,TX_Message.val32);                            
                if(AddSource.val32 == Students_Info[I].ID)
                {
                    if(Rx_Message.val32 == I_AM_LINK)
                    {
                        Students_Info[I].IsOk = true;                       
                    }
                    else
                    {
                        Students_Info[I].IsOk = false; 
                     }
                }
                else
                {

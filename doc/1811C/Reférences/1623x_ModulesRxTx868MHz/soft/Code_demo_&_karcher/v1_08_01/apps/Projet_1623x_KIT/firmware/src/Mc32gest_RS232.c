...
#define FIFO_RX_SIZE ( (10*7) + 1)  // 10 messages
#define FIFO_TX_SIZE ( (10*7) + 1)  // 10 messages
...
int GetMessage(S_UART_message *pData) {
    ...
    {
        GetCharFromFifo (&descrFifoRX, &CarLu2);
        
        if (CarLu2 == (int8_t)0xAA) {
            startOk = true;
            Cycle = 0;
            break;
        }
    ...
    if (startOk) {
        ...
        pData->com = CarLu2;
        GetCharFromFifo(&descrFifoRX, &CarLu2);
        RxMess.add.shl.msb = CarLu2;
         GetCharFromFifo(&descrFifoRX, &CarLu2);
        RxMess.add.shl.byte2 =CarLu2;
         GetCharFromFifo(&descrFifoRX, &CarLu2);
        RxMess.add.shl.byte1 = CarLu2;
        GetCharFromFifo(&descrFifoRX, &CarLu2);
        RxMess.add.shl.lsb = CarLu2;
